...
namespace GymSample2
{
    public partial class TrainerSignUpForm : Form
    {
        ...
        private void signupbtn_Click(object sender, EventArgs e)
        {
            ...
            int age;
            bool isAgeValid = int.TryParse(txtage.Text, out age);  // Validate age input
            string phoneNumber = txtphonenumber.Text;
            string userName = txtusername.Text;
            string password = txtpassword.Text;
            string trainerspecialization = txttrainerspecialization.SelectedItem.ToString();

            Trainer trainer = new Trainer(name, email, age, phoneNumber, userName, password, trainerspecialization);

            double salary = trainer.CalculateSalary();
            ...
            if (validateTrainerInput(name, email, age, phoneNumber, userName, password, out validationMessage))
            {
                ...
                int newTrainerID = TrainerManager.CreateTrainer(name,email,age,phoneNumber,userName,password,trainerspecialization,salary);

                // Check if the creation was successful
                if (newTrainerID > 0)
                {
                    MessageBox.Show($"Trainer created successfully! Your Trainer ID is {newTrainerID}.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);

                    // Redirect to TrainerSignInForm
                ...
                {
                    MessageBox.Show("There was an error creating your account. Please try again.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            else
            {
